...
//
// ### function etcd (key, options)
// #### @key {string|Object}  Fully qualified options, name of etcd store/key.
// #### @path {object} **Optional** Must include "cluster": array of etcd nodes and "key": path in the etcd directory
// Adds a new `Etcd` store to this instance. Accepts the following options
//
//    nconf.etcd({cluster:{ host: '127.0.0.1', port: 4001 } ], key: "mysql" });
//    nconf.etcd('userconfig', {cluster:{ host: '127.0.0.1', port: 4001 } ], key: "mysql", readOnly: true, sync: true});

Provider.prototype.etcd = function (key, options) {
    if (arguments.length == 1) {
        options = key;
        key = 'etcd';
    }
...
//
// ### function watch (store, callback)
// #### @store {string} **Optional** Selects a store to observe. If no store is selected, it observes them all.
// Returns an EventEmitter, which can be used to listen on changes in one or all store(s).
//
Provider.prototype.watch = function (store) {
    self = this;

    eventEmitter = new EventEmitter();

    if (store !== null && store !== undefined) {
        this.stores[store].on('change', function (message) {
            eventEmitter.emit('change', message);
        });
    }
    else {
        Object.keys(this.stores).forEach(function (store) {
            self.stores[store].on('change', function (message) {
                eventEmitter.emit('change', message);
            });
        });
    }
...
Provider.prototype._execute = function (action, syncLength /* [arguments] */) {
    ...
    var args = Array.prototype.slice.call(arguments, 2),
        ...
        response,
        mergeObjs = [],
        targetStore = args.length >= 3 && args[args.length - 1] !== undefined ? args.shift() : null;
    ...
    Object.keys(this.stores).forEach(function (name) {
        if (targetStore !== null && targetStore !== name) {
            return;
        }
